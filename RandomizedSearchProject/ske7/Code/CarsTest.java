 ...
 * Edited by Shannon on 11/4/2018
 */
...
//  TODO: Your dataset filename. Expected format is CSV
  private static final String FILENAME = "C:/Users/Crymsonfire/Documents/_3rdyearfall/ML/RandomizedSearchProject/datasets/processed-cars-data.csv";

  //  TODO: How many examples you have
  private static int num_examples = 1728;
...
//  TODO: Set this to false if you retained the best SA params from a previous run
  ...
  private static double[] temps = {1e3, 1e5, 1E7, 1E9, 1E11, 1E13, 1E15};
  //private static double[] coolingRates = {0.9, 0.95, 0.99, 0.999};
  private static double[] coolingRates = {0.95};
...
//         Run actual SA with best params here
 //       trainIterations = 2000;
        runSA(trainIterations, 1e7, 0.50);
...
//        Run actual GA with best params here
        trainIterations = i * 10;
//        for (double i = 0; i <= 0.91; i += 0.1) {
//        	System.out.print(i);
//        	runGA(trainIterations, 0.1, i);
//        }
        runGA(trainIterations, 0.04, 0.02);
    }
...
//    Convert nanoseconds to seconds
  ...
  public static void determineSAParams(int trainIterations) {
    ...
    double[][] valerrors = new double[temps.length][coolingRates.length];
    double[][] trerrors = new double[temps.length][coolingRates.length];
    double[][] teerrors = new double[temps.length][coolingRates.length];
    ...
    for (int x = 0; x < temps.length; x++) {
      ...
      for (int y = 0; y < coolingRates.length; y++) {
        ...
        double[] validationErrors = new double[K];
        double[] trainError = new double[K];
        double[] testError = new double[K];
        for (int i = 0; i < nets.length; i++) {
...
//          trainError[i] = evaluateNetwork(backpropNet, trainSet.getInstances());
//          testError[i] = evaluateNetwork(backpropNet, testSet.getInstances());
...
//      Find the average error for this network configuration
        ...
        double teerror = 0;
        for (int j = 0; j < validationErrors.length; j++) {
          valerror += validationErrors[j];
          trerror += trainError[j];
          teerror += testError[j];
        }
        valerror /= validationErrors.length;
        trerror /= validationErrors.length;
        teerror /= validationErrors.length;
        valerrors[x][y] = valerror;
        trerrors[x][y] = trerror;
        teerrors[x][y] = teerror;
    ...
    for (int x = 0; x < temps.length; x++) {
      ...
      for (int y = 0; y < coolingRates.length; y++) {
    	  //System.out.println("Errors for each temperature: ");
//    	  System.out.print("train error: ");
//    	  System.out.println(trerrors[x][y]);
    	  train += trerrors[x][y] + ",";
//    	  System.out.print("test error: ");
//    	  System.out.println(teerrors[x][y]);
    	  test += teerrors[x][y] + ", ";
//    	  System.out.print("val error: ");
//    	  System.out.println(valerrors[x][y]);
    	  val += valerrors[x][y] + ", ";
        if (minErr > valerrors[x][y]) {
          best_temp_index = x;
          best_cool_index = y;
          minErr = valerrors[x][y];
        }
      }
    }
...
//      Find the average error for this configuration
   ...
   */
  public static void runGA(int trainIterations, double mateRatio, double mutateRatio) {
    ...
    int populationSize = (int) (trainSetSize);
    int toMate = (int) (trainSetSize * mateRatio);
    int toMutate = (int) (trainSetSize * mutateRatio);
