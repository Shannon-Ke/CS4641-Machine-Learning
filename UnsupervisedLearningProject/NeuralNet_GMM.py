# -*- coding: utf-8 -*-
"""
Created on Mon Nov 19 21:11:18 2018
...
gmm.fit(W_train)
gmm_predict = gmm.predict(W_train)
#with open('datasets/cars-data.csv','r') as csvinput:
#    with open('datasets/cars-data-gmm.csv', 'w') as csvoutput:
#        writer = csv.writer(csvoutput, lineterminator='\n')
#        reader = csv.reader(csvinput)
#
#        all = []
#        row = next(reader)
#        row.append(gmm_predict[i])
#        all.append(row)
#
#        for row in reader:
#            row.append(row[0])
...
y = []
car_data = pd.read_csv("datasets/cars-data-gmm.csv")
    ## One Hot Encoding my Data
...
plt.title('GMM with Cars Data on a NN')